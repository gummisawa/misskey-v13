...
<div class="_autoGap">
	...
	<FormFolder>
		...
		<div class="_autoGap"> <!--TODO admin処理あたりを参考にisCatとisFoxを重複して設定できないようにするべきかも, テキストはisCatとかと同じように言語ごとに分けて適切な(?)ファイルに書くべきかも-->
			<FormSwitch v-if="!$i?.isFox" v-model="profile.isCat" class="_formBlock">{{ i18n.ts.flagAsCat }}<template #caption>{{ i18n.ts.flagAsCatDescription }}</template></FormSwitch>
			<FormSwitch v-if="!$i?.isCat" v-model="profile.isFox" class="_formBlock">{{ i18n.ts.flagAsFox }}<template #caption>{{ i18n.ts.flagAsFoxDescription }}</template></FormSwitch> 
